...
public record WoodSet(String blockID, BlockSetType blockSetType, WoodType woodType,
	...
	public BlockEntry<Block> wallSign() {
		ResourceLocation key = sign().resourceKey().location().withPath(p -> p.replace("sign", "wall_sign"));
		return new BlockEntry<>(key, BuiltInRegistries.BLOCK.get(key));
	}

	public BlockEntry<Block> hangingWallSign() {
		ResourceLocation key = hangingSign().resourceKey().location().withPath(p -> p.replace("sign", "wall_sign"));
		return new BlockEntry<>(key, BuiltInRegistries.BLOCK.get(key));
	}
}
